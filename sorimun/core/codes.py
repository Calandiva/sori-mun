...
멜로디의 짜임 — 낱말마다 제 조(으뜸음)를 갖는다.

    으뜸음   57~61 (A3~C#4) 다섯 조. 낱말의 정체에서 결정적으로 나온다.
    자릿음   으뜸음 위 그 성질의 음계를 지그재그 순열로 밟는 홑음들.
             자릿값이 리듬(3~5)까지 만든다.
    종지     으뜸음 위 3도(장3도=장, 단3도=단, 삼전음=중성)로 올라섰다가
             아래로 해결한다. 하행 도약의 거칢이 등급이다 — 익숙하면
             완전4도로 순하게, 생소하면 단2도의 긴장으로.
...
# ── 이름 멜로디 ──────────────────────────────────────────────────────
TONICS = tuple(range(57, 62))       # A3~C#4 — 다섯 조
MEL_BASE = 57                       # (안내·표시용)


def tonic_of(tier: int, qi: int, index: int) -> int:
    """낱말의 으뜸음. 같은 낱말은 언제나 같은 조를 갖는다."""
    return TONICS[(index + 3 * max(0, tier) + 5 * qi) % len(TONICS)]
